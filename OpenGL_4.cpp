...
GLuint loadimage(const char* fileName)// texture 
{
	FILE* file;
...
void init()
{
	nen = loadimage("co.bmp");
	glClearColor(0, 0, 0, 0);
	glEnable(GL_DEPTH_TEST);
	glShadeModel(GL_FLAT);
}

void drawland()//ve nen
{
	glEnable(GL_TEXTURE_2D);
	glDisable(GL_LIGHTING);
	glBindTexture(GL_TEXTURE_2D, nen);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);
	//glColor3f(1.0, 0.0, 0.0);
	glBegin(GL_QUADS);
	glVertex3d(50, -7, 50);     glTexCoord2f(0, 0);
	glVertex3d(50, -7, -50);    glTexCoord2f(0, 1);
	glVertex3d(-50, -7, -50);   glTexCoord2f(1, 1);
	glVertex3d(-50, -7, 50);    glTexCoord2f(1, 0);
	//glEnd();
...
void init_func()
{
	...
	glEnable(GL_DEPTH_TEST);
	// thiết lập ánh sáng
	GLfloat ambient_lighte[4] = { 0.5,0.5,0.5,1.0 }; //anh sáng môi trường
	GLfloat diffuse_light[4] = { 0.7,0.7,0.7,1.0 };		// ánh sáng khuếch tán
	GLfloat specular_light[4] = { 1.0, 1.0, 1.0, 1.0 };	//ánh sáng phản chiếu
	GLfloat light_position[4] = { 0.0, 0.0, 1.0, 1.0 };  //vị trí  chiếu sáng
	...
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, ambient_lighte);
	glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specularity);
	glMateriali(GL_FRONT_AND_BACK, GL_SHININESS, material_specularity);

	//gọi ánh sáng xung quanh
...
void mymenu(int id) {
	...
	case 1:
		...
		break;
	case 2:
		PlaySound(TEXT("2.wav"), NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
		break;
	case 3:
		PlaySound(TEXT("3.wav"), NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
		break;
	case 4:
		PlaySound(TEXT("4.wav"), NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
		break;
	case 5:
		PlaySound(TEXT("5.wav"), NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
		break;
	case 6:
		PlaySound(TEXT("6.wav"), NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
		break;
	case 7:
		PlaySound(TEXT("7.wav"), NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
		break;
	case 8:
...
int main(int argc, char** argv)
{
	...
	glutAddMenuEntry("Nhac thieu nhi", 1);
	glutAddMenuEntry("Nhac khong loi", 2);
	glutAddMenuEntry("Nhac tru tinh", 3);
	glutAddMenuEntry("Nhac buon", 4);
	glutAddMenuEntry("Nhac soi dong", 5);
	glutAddMenuEntry("Nhac Trinh", 6);
	glutAddMenuEntry("Nhac bong da", 7);
	glutAddMenuEntry("Tat nhac", 8);
	glutAddMenuEntry("Dung choi", 9);
